...
$(document).on('click','.home-del-btn',function(e){
	...
	var cusids = $('body').find('#inf').val();
	var cus_ids = $('body').find('#c_ids').val();
...
$(document).on('click','.family-edit-btn',function(data){
		...
		$.post('../pscripts/customer/family_tab_query',{ id : $(this).attr('data-value') }, function(data)
		{
			...
			var parsePhone = $.parseJSON(parse.cphone);
			var parsePhoneType = $.parseJSON(parse.cptype);
			
			var Work,Home,Business,Cellular = '';
			if (parsePhoneType instanceof Array) {
			for(var cptype = 0; cptype < parsePhoneType.length; cptype++)
			{
					console.log(parsePhone[cptype]);
				
					if(parsePhoneType[cptype]==30){
						 Work = "selected";
					}
					if(parsePhoneType[cptype]==40){
						 Home = "selected";
					}
					if(parsePhoneType[cptype]==50){
						 Business = "selected";
					}
					if(parsePhoneType[cptype]==60){
						 Cellular = "selected";
					}
					...
					+'<div class="form-group">'
							...
							+'<div class="col-sm-5" style="padding: 0;">'
								+'<div class="form-group">'
									+'<select id="familyphonetype" name="familyphonetype[]" class="form-control">'
										+'<option value="0" selected>Phone Type</option>'
										+'<option value="30" '+Work+'>Work</option>'
										+'<option value="40" '+Home+'>Home</option>'
										+'<option value="50" '+Business+'>Business</option>'
										+'<option value="60" '+Cellular+'>Cellular</option>'
									+'</select>'
								+'</div>'
							+'</div>'
							+'<div class="col-sm-7">'
								+'<div class="form-group">'
									+'<div class="input-group">'
										+'<div class="input-group-content">'
											+'<input type="text" class="form-control" placeholde="Tel/Cell Number" maxlength="10" name="familyphone[]" id="familyphone" value="'+parsePhone[cptype]+'">'
										+'</div>'
									+'</div>'
								+'</div>'
							+'</div>'
						+'</div>'
					+'</div>'
					+'<span class="remove_phone_group_fam_del" data-attr="'+parse.cpid+'" style="position: absolute;right: 3em;"><i class="md md-remove-circle"></i></span></div>';//New input field html 
					$(wrap).append(field);
					$(':input').trigger('change');
				$('select').trigger('change');
			}
						...
						+'<div class="col-lg-12" style="padding: 0;">'
							...
							+'<div class="col-sm-7">'
								+'<div class="form-group">'
									+'<div class="input-group">'
										+'<div class="input-group-content">'
											+'<input type="text" class="form-control" placeholde="Tel/Cell Number" maxlength="10" name="familyphone[]" id="familyphone" value="'+parse.cphone+'">'
										+'</div>'
									+'</div>'
								+'</div>'
							+'</div>'
			...
			$(':input').trigger('change');
			$('select').trigger('change');
			// $('#familyphonetype').val(parse.cptype);
